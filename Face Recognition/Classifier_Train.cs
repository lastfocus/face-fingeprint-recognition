...
class Classifier_Train: IDisposable
{
    ...
    public Classifier_Train()
    {
        _IsTrained = LoadData(Application.StartupPath + "\\TrainedFaces");
    }
    /// <summary>
   ...
   /* public Classifier_Train(string Training_Folder)
    ...
    public Classifier_Train(string Training_Folder)
    {
        _IsTrained = LoadData(Training_Folder);
    }
    #endregion
   ...
   /* public bool Retrain()
    ...
    public bool Retrain()
    {
        return _IsTrained = LoadData(Application.StartupPath + "\\TrainedFaces");
    }
    /// <summary>
    /// Retrains the recognizer witout resetting variables like recognizer type.
    /// Takes String input to a different location for training data.
   ...
   /* public bool Retrain(string Training_Folder)
    ...
    public bool Retrain(string Training_Folder)
    {
        return _IsTrained = LoadData(Training_Folder);
    }
    /// <summary>
...
if (_IsTrained)
{
    ...
    else
    {
        ...
        switch (Recognizer_Type)
        {
            ...
            case ("EMGU.CV.EigenFaceRecognizer"):
                if (Distance > Eigen_threshold) return label;
                else return "Unknown";
...
{
    con.Open();              
    using (SqlCeCommand com = new SqlCeCommand("SELECT * FROM faces",con))
    {
        ...
        while (reader.Read())
        {
            ...
            image.Save(path.Substring(0, path.Length - 3),ImageFormat.Jpeg);
            cs.Close();
            fsCrypt.Close();                            
        }
    }   
